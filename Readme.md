# Track Evaluation (TrackEval)

This repository provides the **Track Evaluation Software Module** used to quantitatively assess multi-object tracking (MOT) results produced by the Audio-Visual Sensor Fusion pipeline. It is based on the official **TrackEval** reference implementation and is used to evaluate predicted speaker tracks against ground-truth trajectories using state-of-the-art MOT metrics.
...
The Track Evaluation module compares:

- **Predicted tracks** generated by the Sensor Fusion module  
  → `fusion_tracks.jsonl`  
- **Ground-truth tracks** exported from the simulation  
  → `groundtruth_sources.jsonl`


This module is used to produce the final quantitative results reported in the master’s thesis.

---

## Evaluation Methodology

The evaluation is performed using the **TrackEval HOTA reference implementation**, as described in:

- Luiten et al., *HOTA: A Higher Order Metric for Evaluating Multi-Object Tracking*, IJCV 2021.

TrackEval decomposes tracking performance into:
...
This decomposition enables fine-grained error analysis across different sensor modalities.
Only the **ground-truth tracks** and **predicted tracks** are required by TrackEval. All other files provide contextual information and are used by upstream modules.
...
### Ground Truth
- e.g. `data\gt\mot_challenge\MOT25A-train\MOT25A-01\gt\gt.jsonl`  
  Time-stamped 3D trajectories of all people/speakers in world coordinates.

### Predictions
- e.g. `data\trackers\mot_challenge\MOT25AV-train\MS-GLMB-GMS\data\MOT25AV-01.jsonl`  
  Predicted speaker tracks produced by the Audio-Visual Sensor Fusion module.

Both files follow a unified JSONL format, where each line represents a time-stamped state of a tracked object.

---

## Output

TrackEval produces per-sequence and aggregated metrics, including:

- HOTA score curves
- Summary tables (mean over all sequences)
- Optional per-frame and per-track diagnostics

The results are used directly for quantitative comparison between:
- Audio-only tracking
- Video-only tracking
