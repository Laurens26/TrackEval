...
class AVMotChallenge(_BaseDataset):
    """Dataset class for Audio-Visual MOT Challenge tracking"""
    ...
    def __init__(self, config=None):
        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                ...
                for seq in self.seq_list:
                    curr_file = os.path.join(self.tracker_fol, tracker, self.tracker_sub_fol, seq + '.jsonl')
                    if not os.path.isfile(curr_file):
                        print('Tracker file not found: ' + curr_file)
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        """
        Load GT or tracker results from JSONL files with 3D positions.

        Expected GT JSONL: objects with keys {"object_id": int, "position": [x,y,z]}
        Expected Tracker JSONL: objects with keys {"track_id": int, "position": [x,y,z]}
        """
        # File location
        if self.data_is_zipped:
            if is_gt:
            ...
            zip_file = None
            if is_gt:
                file = "{gt_folder}/{seq}/gt/gt.jsonl".format(gt_folder=self.gt_fol, seq=seq)
                loaded = self._load_jsonl_xyz_file(file, id_key="object_id")
            else:
                file = os.path.join(self.tracker_fol, tracker, self.tracker_sub_fol, seq + '.jsonl')
                loaded = self._load_jsonl_xyz_file(file, id_key="object_id")

        num_timesteps = loaded['num_timesteps']
        dets_list = loaded['dets']   # list of (N_t,3) arrays
        ids_list = loaded['ids']     # list of (N_t,) arrays

        # Prepare the raw_data dict with the same keys as the reference loader
        if is_gt:
            raw_data = {
                'gt_dets': dets_list,
                'gt_ids': ids_list,
                'gt_classes': [np.ones_like(ids) for ids in ids_list],  # all class=1
                'gt_crowd_ignore_regions': [np.empty((0, 3)) for _ in range(num_timesteps)],  # no ignore regions
                'gt_extras': [{'zero_marked': np.ones_like(ids)} for ids in ids_list],
                'num_timesteps': num_timesteps,
                'seq': seq,
            }
        else:
            raw_data = {
                'tracker_dets': dets_list,
                'tracker_ids': ids_list,
                'tracker_classes': [np.ones_like(ids) for ids in ids_list],  # all class=1
                'tracker_confidences': [np.ones_like(ids, dtype=float) for ids in ids_list],  # dummy conf=1.0
                'num_timesteps': num_timesteps,
                'seq': seq,
            }
    ...
    def _load_jsonl_xyz_file(self, path, id_key, pos_key="position"):
        # Returns same shape as _load_mot_xyz_file
        records = []
        ...
        # Sort by timestamp; map to 1-based frames
        records.sort(key=lambda r: float(r["timestamp"]))
        frame_of = {r["timestamp"]: i+1 for i, r in enumerate(records)}
        max_frame = len(records)

        dets_list = [None] * max_frame
        ids_list  = [None] * max_frame

        for r in records:
            t = frame_of[r["timestamp"]] - 1
            objs = r.get("objects", [])
            xyz = []
            ids = []
            for o in objs:
                if id_key not in o:
                    raise ValueError(
                        f"Missing '{id_key}' in JSONL object at timestamp {r['timestamp']}. "
                        "Every object must carry a persistent ID."
                    )
                ids.append(int(o[id_key]))
                x, y, z = map(float, o[pos_key])
                xyz.append([x, y, z])
            import numpy as np
            dets_list[t] = np.asarray(xyz, dtype=np.float32) if xyz else np.zeros((0,3), np.float32)
            ids_list[t]  = np.asarray(ids, dtype=np.int64)   if ids else np.zeros((0,), np.int64)

        return {'dets': dets_list, 'ids': ids_list, 'num_timesteps': max_frame}
    ...
    def _calculate_similarities(self, gt_dets_t, tracker_dets_t):
        ...
        # HARCODED WORKAROUND (make tracks 2D instead of 3D)
        # Make groundtruth and detections 2d instead of 3d 
        if gt_dets_t.size > 0:
            gt_dets_t = gt_dets_t[:, :2]  # shape 1x2
        else:
            gt_dets_t = np.empty((0, 2))

        if tracker_dets_t.size > 0:
            tracker_dets_t = tracker_dets_t[:, :2]  # shape Nx2
        else:
            tracker_dets_t = np.empty((0, 2))

        # Use euclidean distance as similarity_score
        # similarity is defined [0 ... 1] and used while computing HOTA for all similarity thresholds [0.05, ..., 0.95] (see array_labels in HOTA class)
        # zero_distance: defines the distance at which similarity drops to 0.
        # e.g. zero_distance = 2.0  means:  euclidian dist(2.0m) -> similarity = 0.0 
        #                                   euclidian dist(1.0m) -> similarity = 0.5
        #                                   euclidian dist(0.0m) -> similarity = 1.0
        # e.g. zero_distance = 1.0  means:  euclidian dist(2.0m) -> similarity = 0.0 
        #                                   euclidian dist(1.0m) -> similarity = 0.0
        #                                   euclidian dist(0.0m) -> similarity = 1.0
        similarity_scores = self._calculate_euclidean_similarity(gt_dets_t, tracker_dets_t, zero_distance=2.0)
